...
ifnb.list <- Seurat::SplitObject(object = ifnb, split.by = "stim")
```
```{r}
ifnb.list
```
...
features <- Seurat::SelectIntegrationFeatures(object.list = ifnb.list)
```
```{r}
head(features)
length(features)
...
immune.combined <- Seurat::RunPCA(immune.combined, npcs = 30, verbose = FALSE)
...
```{r}
Seurat::FeaturePlot(immune.combined, features = c("CD3D", "SELL", "CREM", "CD8A", "GNLY", "CD79A", "FCGR3A",
